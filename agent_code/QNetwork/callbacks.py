...
def act(self, game_state):
    ...
    # Check if the q_network is set properly
    if not hasattr(self, 'q_network'):
        raise AttributeError("q_network is not initialized")

    # Use the policy network to choose the best action
    action_index = self.q_network(state).argmax().item()
